...
Веб-ресурсы фиксируются отдельными GitHub-коммитами в [`data/snapshots`](data/snapshots). Поэтому каждая карточка найденной сборки содержит проверяемую ссылку вида `GitHub · jutsu-dev/UnixGramChangelog@<commit>`.
